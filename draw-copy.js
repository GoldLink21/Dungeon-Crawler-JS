...
function drawAllInGame(){
    ...
    board.forEach(y=>{
        ...
        y.forEach(x=>{
            ...
            if(x.is(Tn.portal)&&(debug.showPortalId||debug.showCoords)){
                ...
                ctx.fillText(x.id,((bx+1)*Tn.SIZE-12),((by+1)*Tn.SIZE-3))
                ctx.fillStyle='black'
            }
            //Draw coords on tiles
            if(debug.showCoords){
                ctx.fillStyle='rgba(10,10,10,0.5)'
                ctx.font=10+'px Times New Roman'
                ctx.fillText("("+bx+','+by+')',(bx*Tn.SIZE+2),(by*Tn.SIZE+10))
    ...
    ctx.font=15+'px Times New Roman'
    if(doDebug)
